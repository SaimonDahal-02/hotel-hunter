...
def check_case(target_id, trip_type, companion, db):
    ...
    if len(target_user) == 0:
        ...
        if (trip_type is None) & (companion is None):
            status = [0, "case_none"]
            data_out = get_main(
                target_id, target_user, other_user, trip_type, companion, status[0]
            )
            del data_out["others"]
            ...
            num_target = len(
                target_user.loc[
                    (target_user["trip_type"] == trip_type)
...
def get_main(target_id, target, other, trip_type, companion, status):
    if status == 1:
        target = target.loc[
            (target["trip_type"] == trip_type) & (target["companion"] == companion)
        ]
...
def re_get_main(data_main, target_id, user_data, trip_type, companion):
    target_data = user_data.loc[
        (user_data["user_id"] == target_id)
        & (user_data["trip_type"] == trip_type)
...
def prediction_rating(dataset, data_frame, **kwargs):
    ...
    if (kwargs["trip_type"] is None) & (kwargs["companion"] is None):
        ...
        for neighbor in neighbors:
            ...
            try:
                result = dataset.ix[
                    (dataset["user_id"] == neighbor)
                    & (dataset["trip_type"] == kwargs["trip_type"])
                    & (dataset["companion"] == kwargs["companion"]),
                    ["user_id", "hotel_name", "rating", "trip_type", "companion"],
                ]
            except:
                pass
    ...
    data_rec = pd.merge(
        data_rec.drop(["trip_type", "companion"], axis=1),
        data_frame,
        how="inner",
        on="user_id",
    )
    ...
    for hotel, group in groups:
        predict_rating = group.apply(
            lambda row: (row.loc["weight"] * row.loc["rating"]) / group["weight"].sum(),
            axis=1,
        ).sum()
        result1.append(hotel)
        result2.append(predict_rating)
    hotel_rec = pd.DataFrame(data={"hotel_name": result1, "predict_rating": result2})
    return hotel_rec


def recommendation(target_id, data_rec, top_k):
    topk_hotels = data_rec.sort_values(by=("predict_rating"), ascending=False).head(
        top_k
    )
...
def search(request):
    ...
    if ok:
        ...
        data_in = pd.read_sql(
            "select * from ratings where province = '" + des + "'", con=db
        ).drop("id", axis=1)
    ...
    cursor.execute(
        'select count(*) from hotels where destination="'
        + des
        + '"'
        + filter_price
        + filter_rate
    ...
    try:
        ...
        for i in range((page - 1) * 6, last):
            ...
            cursor.execute(
                'select image from hotel_images where hotel_name="' + this[1] + '"'
            )
            img = cursor.fetchone()
            if img is None:
                img = [""]
    ...
    for k in list_rec:
        try:
            ...
            adv = pd.read_sql(
                'select price_comp, near_station, restaurant, entertain, shopping_mall, convenience_store from hotels where destination="'
                + des
                + '" and name="'
                + k
                + '"',
            ...
            adv = (
                adv.transpose()
                .sort_values(0, axis=0, ascending=False)
                .transpose()
                .values[1]
                .tolist()
            )
            ...
            cursor.execute(
                'select image from hotel_images where hotel_name="' + k + '"'
            )
            img = cursor.fetchone()
            if len(img) == 0:
                img = ""
    ...
    cursor.execute(
        'select distinct district from hotels where destination="' + des + '"'
    )
